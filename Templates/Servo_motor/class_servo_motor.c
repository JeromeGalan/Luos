...
static void ServoMotorConfig_Handler(container_t *container, msg_t *msg, class_servo_motor_t *class_servo_motor)
{
    if (msg->header.cmd == PARAMETERS)
    {
        ...
        memcpy((void *)&class_servo_motor->mode, msg->data, sizeof(servo_motor_mode_t));
...
void ClassServoMotor_Handler(container_t *container, msg_t *msg, class_servo_motor_t *servo_motor_class)
{
    switch (msg->header.cmd)
    {
        ...
        {
            // fill the message infos
            if ((servo_motor_class->mode.mode_angular_position || servo_motor_class->mode.mode_linear_position) && !(servo_motor_class->mode.mode_angular_speed || servo_motor_class->mode.mode_linear_speed))
            {
                // only position control is enable, we can save PID for positioning
                PidOD_PidFromMsg(&servo_motor_class->position_pid, msg);
            }
            if ((servo_motor_class->mode.mode_angular_speed || servo_motor_class->mode.mode_linear_speed) && !(servo_motor_class->mode.mode_angular_position || servo_motor_class->mode.mode_linear_position))
            {
                // only speed control is enable, we can save PID for speed
                PidOD_PidFromMsg(&servo_motor_class->speed_pid, msg);
            }
        }
        break;
        case CONTROL:
        ...
        case ANGULAR_SPEED_LIMIT:
        {
            ...
            AngularOD_SpeedFromMsg((angular_speed_t *)&servo_motor_class->limit_angular_speed, msg);
            return;
        }
        break;
        ...
        case LINEAR_SPEED_LIMIT:
        {
            ...
            if (servo_motor_class->mode.mode_linear_position & (servo_motor_class->wheel_diameter != 0))
            {
                ...
                // Directly convert it
                servo_motor_class->limit_angular_speed[0] = (linear_speed[0] * 360.0) / (3.141592653589793 * servo_motor_class->wheel_diameter);
                servo_motor_class->limit_angular_speed[1] = (linear_speed[1] * 360.0) / (3.141592653589793 * servo_motor_class->wheel_diameter);
            }
        }
        break;
        case TIME:
    ...
    ClassMotor_Handler(container, msg, &servo_motor_class->motor);
}
