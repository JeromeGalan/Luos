...
static void TemplateVoltage_MsgHandler(container_t *container, msg_t *msg)
{
    ...
    ClassVoltage_Handler(container, msg, &voltage_template->class);
...
container_t *TemplateVoltage_CreateContainer(CONT_CB cont_cb, template_voltage_t *voltage_template, const char *alias, revision_t revision)
{
    ...
    container->access           = voltage_template->class.access;
    return container;
}
/******************************************************************************
 ...
 * @param voltage_class the class struct to update
 * @return None
 ******************************************************************************/
void ClassVoltage_Handler(container_t *container, msg_t *msg, class_voltage_t *voltage_class)
{
    if ((msg->header.cmd == ASK_PUB_CMD) && ((voltage_class->access == READ_WRITE_ACCESS) || (voltage_class->access == READ_ONLY_ACCESS)))
    {
        // fill the message infos
        msg_t pub_msg;
        pub_msg.header.target_mode = ID;
        pub_msg.header.target      = msg->header.source;
        if (voltage_class->control.rec)
        {
            LUOS_ASSERT(voltage_class->signal.data_ptr != 0);
            // send back a record stream
            pub_msg.header.cmd = VOLTAGE;
            Luos_SendStreaming(container, &pub_msg, &voltage_class->signal);
        }
        else
        {
            ElectricOD_VoltageToMsg(&voltage_class->voltage, &pub_msg);
            Luos_SendMsg(container, &pub_msg);
        }
    }
    if ((msg->header.cmd == VOLTAGE) && ((voltage_class->access == READ_WRITE_ACCESS) || (voltage_class->access == WRITE_ONLY_ACCESS)))
    {
        if (msg->header.size == sizeof(voltage_t))
        {
            // Get the voltage
            ElectricOD_VoltageFromMsg(&voltage_class->voltage, msg);
        }
        else
        {
            LUOS_ASSERT(voltage_class->signal.data_ptr != 0);
            // this is a signal, save it into ring buffer.
            Luos_ReceiveStreaming(container, msg, &voltage_class->signal);
    ...
    {
        // save time in ms
        TimeOD_TimeFromMsg((time_luos_t *)&voltage_class->sampling_period, msg);
        return;
    }
}
